...
{\bf Número de páginas (sin incluir introducción, bibliografía ni anexos):} 50 \\
...
\begin{proof}
    En el espacio de Banach $(\mathcal{C}([-\pi,\pi]),\|\cdot\|_\infty)$, consideremos la familia de aplicaciones $\{T_n\}_{n\in \N}$, donde $T_n \colon \mathcal{C}([-\pi,\pi]) \to \R$, $T_n(g) = S_ng(0)$. Utilizando \hyperref[1.2.4]{(1.2.4)} y teniendo en cuenta que $D_n$ es par,
...
\begin{proof}
    ...
    Veamos que $n\angles{k\alpha} = \angles{nk\alpha}$, lo que finalizará la prueba de \hyperref[2.2.4]{(2.2.4)}. Por definición de la parte fraccionaria,
...
\begin{proof}
    ...
    donde $\widetilde{\varepsilon} = \min\{\varepsilon,1-x\} > 0$. Tenemos entonces $\angles{2n_0\alpha} - (x-\angles{\alpha}) <\widetilde{\varepsilon} \leq 1-x$, luego $0 \leq \angles{2n_0\alpha}+\angles{\alpha} < 1$ y por tanto $E(\angles{2n_0\alpha}+\angles{\alpha}) = 0$. Llevando esto a \hyperref[2.2.8]{(2.2.8)}, concluimos
...
\begin{proof}
    ...
    Para que el intervalo $I_j$ tenga sentido, tomamos $\delta < \frac{\pi}{2n+1}$, tal y como se vio en el \hyperref[2.2.10]{Lema 2.2.10}. Se observa que
    ...
    \begin{itemize}
        ...
        \end{align*}
        Esto completa la prueba de \hyperref[2.2.16]{(2.2.16)}.
        ...
        Sea $j \in \{0,1,\mathellipsis,E(n-\sqrt{n})\}$ y sea $x \in I_{2j} \cup I_{2j+1}$. El \hyperref[2.2.10]{Lema 2.2.10} permite afirmar que existe $k_x \in \N$ tal que $2k_x+1$ es múltiplo de $2n+1$, $m_{j} \leq k_x < \frac{1}{2}m_{j+1}$ y $\sen((k_x+\frac{1}{2})x) < -\frac{1}{2}$.
        ...
        \end{align*}
        Usando esto y \hyperref[2.2.16]{(2.2.16)},
        \begin{align*}
       ...
       Si $j+1 \leq i \leq n$, entonces $k_x < \frac{1}{2}m_{j+1} \leq \frac{1}{2}m_i$, luego $\frac{k_x}{m_i} \leq \frac{1}{2}$ y por tanto
        ...
        \end{aligned}\]
        Como $j \leq n-\sqrt{n}$, entonces $n-j \geq \sqrt{n}$ y por tanto $\lim_{n\to\infty}(n-j) = \infty$. Usando esto y el \hyperref[2.2.12]{Lema 2.2.12}, 
        ...
        \]
        utilizándose que $\phi_n$ es un polinomio trigonométrico no negativo de grado $m_0 \leq k_x$. Con esto queda probado \hyperref[2.2.17]{(2.2.17)}.
        \item \textit{Si $n$ es suficientemente grande, para todo $x \in \bigcup_{j=0}^{2n}I_j'$ se verifica \begin{equation}\label{2.2.18}
        ...
        En cualquier caso,
        \[|x-x_{2i}| \geq \frac{\pi |l -2i|}{2n+1}.\]
        Poniendo $k = m_0$ y $j = 0$ en la desigualdad \hyperref[2.2.16]{(2.2.16)} (es claro que $m_j \leq k < m_{j+1}$ y por tanto dicha desigualdad es válida), tenemos
        ...
        \end{align*}
        Usando de nuevo el \hyperref[2.2.12]{Lema 2.2.12},
        ...
        Supongamos ahora que $l$ es par, es decir, que existe $i_0 \in \{0,1,\mathellipsis,n\}$ tal que $l = 2i_0$. En tal caso, volviendo a usar \hyperref[2.2.16]{(2.2.16)} con $k = m_0$ y $j = 0$,
        \begin{align*}
            S_{m_0}f_n(x) &\geq \frac{1}{2n}\sum_{i=1}^n \frac{m_i-m_0}{m_i+1}D_{m_0}(x-x_{2i}) \\
        ...
        \end{align*}
        Utilizando \hyperref[2.2.15]{(2.2.15)} y que $x-x_l \in I_0'$ por ser $x \in I_l'$, se obtiene $D_{m_0}(x-x_l) \geq 0$, y en consecuencia,
        ...
        \end{align*}
        obteniéndose así \hyperref[2.2.18]{(2.2.18)}. En la segunda igualdad se ha usado que $\phi_n$ es un polinomio trigonométrico de grado $m_0$, y en la primera desigualdad se ha usado \hyperref[2.2.14]{(2.2.14)}.
    \end{itemize}
    ...
    donde $C$ es la constante que aparece en \hyperref[2.2.17]{(2.2.17)}. Es claro que $A_n > 0$ (podemos suponer $n>1$) y que $\lim_{n \to \infty} A_n = \infty$.
    ...
    \begin{itemize}
        ...
        \item Supongamos que $x \in \bigcup_{j=0}^{n-1}(I_{2j}\cup I_{2j+1})$. Sea $j \in \{0,1,\mathellipsis,n-1\}$ con $x \in I_{2j} \cup I_{2j+1}$. Como $x \leq x_{2E(n-\sqrt{n})}$ (porque $x \in E_n$) y $x > x_{2j}$ (porque $x \in I_{2j} \cup I_{2j+1}$), tiene que ser $2j < 2(n-\sqrt{n})$, así que $j < n-\sqrt{n}$ y puede usarse \hyperref[2.2.17]{(2.2.17)} para obtener que existe $k \in \N$ con 
...
\begin{proof}
    Por el \hyperref[3.0.2]{Lema 3.0.2}, hay que probar que para todo $C > 0$ existen $n \in \N$ y $f \in L^p(\T)$ tales que
    \[\|S_nf\|_1 > C\|f\|_1.\]
    Sea $C>0$. Usando el lema anterior y el \hyperref[2.1.1]{Lema 2.1.1}, obtenemos
    \[\lim_{n\to\infty} \|T_n\| = \lim_{n\to\infty}\|D_n\|_1 = \infty,\]
    siendo $T_n \colon L^1(\T) \to L^1(\T)$ la aplicación dada por $T_n(f) = S_nf$. Por el contrarrecíproco del \hyperref[1.3.1]{teorema de la acotación uniforme}, existe $f \in L^1(\T)$ tal que el conjunto $\{T_n(f) \colon n \in \N\}$ no es acotado en $L^1(\T)$. En consecuencia,
...
\begin{proof}
    ...
    El objetivo es aplicar a $\varphi$ el \hyperref[3.2.3]{lema de las tres líneas}. En primer lugar, usando la linealidad de $T$ y de la integral, para todo $z\in\C$ se tiene
    ...
    \end{aligned}\]
    y esto último es cierto por hipótesis. Por tanto, $0 < t < 1$. Por el \hyperref[3.2.3]{lema de las tres líneas}, para todo $z\in\C$ con $\real(z)=t$ se tiene que $|\varphi(z)| \leq M_0^{1-t}M_1^t$. En particular, $|\varphi(t)| \leq M_0^{1-t}M_1^t$. Hallemos $|\varphi(t)|$. Como $r\phi(t) = 1$, se tiene que
    \begin{align*}
    ...
    Llamando $M_r = M_0^{1-t}M_1^t$ y usando \hyperref[3.2.6]{(3.2.6)}, obtenemos que $\|T(f)\|_r \leq M_r$ para toda $f \in S$ con $\|f\|_r = 1$. Por ser $T$ lineal, esto implica,, que $\|T(f)\|_r \leq M_r\|f\|_r$ para toda $f \in S$.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \end{align*}
        Para $k\in\{1,2,\mathellipsis,m-1\}$, aplicamos la \hyperref[1.1.1]{desigualdad de Hölder} con exponentes conjugados $\frac{2m}{2m-2k}$ y $\frac{2m}{2k}$, obteniendo
        \begin{align*}
    ...
    \end{itemize}
    Como $2m<p<2m+2$, por el \hyperref[3.2.5]{teorema de interpolación de Riesz-Thorin}, existe $M_p>0$ tal que
    \[\|T(f)\|_p  \leq M_p\|f\|_p\]
...
\begin{proof}
    ...
    \end{align*}
    Nótese que $G,H\in\mathcal{P}$ y por tanto $G^+$ y $H^+$ tienen sentido. Usando esto junto con \hyperref[3.3.5]{(3.3.5)},
    ...
    \begin{itemize}
        ...
        Por la \hyperref[1.1.1]{desigualdad de Hölder},
